...
inline void Mark(uint8* const marker, const size_type index) noexcept
{
	marker[index / 8] |= 1 << (index % 8);
}

inline void Unmark(uint8* const marker, const size_type index) noexcept
{
	marker[index / 8] &= ~(1 << (index % 8));
}

HeapMemory::HeapMemory(const size_type size) noexcept
{
	check(size != 0);

	maxNum = size;
	markerSize = static_cast<size_type>(
...
void* HeapMemory::Malloc(const size_type n) noexcept
{
	if (n == 0 || maxNum - curNum < n)
		return nullptr;

	for (size_type i = 0, clearSectionNum = 0; i < maxNum; ++i) {
...
void HeapMemory::Free(void* const ptr, const size_type n) noexcept
{
	check(ptr >= memory && ptr < memory + maxNum);

	curNum -= n;
	const size_type startIdx = static_cast<uint8*>(ptr) - memory;
